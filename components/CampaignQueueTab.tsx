...
const BOT_URL = process.env.NEXT_PUBLIC_COMMENT_BOT_URL || "http://localhost:5001";

// ─── Types ────────────────────────────────────────────────────────────────────

type Platform = "instagram" | "x" | "threads" | "tiktok";
type CampaignStatus = "not-started" | "in-progress" | "completed" | "failed" | "aborted";
...
function mapDbToCampaign(row: any): CommentCampaign {
  return {
    id: row.campaign_id,
    platform: row.platform,
    status: row.status,
    comment: row.custom_comment || "",
...
function StatusBadge({ status, withDot }: { status: CampaignStatus; withDot?: boolean }) {
  const map: Record<CampaignStatus, { text: string; cls: string }> = {
    "not-started": {
      text: "Queued",
...
// ─── Custom Dropdown ──────────────────────────────────────────────────────────
// Same floating-list pattern as ConfigureCommentsTab.
    ...
    <div className="border-b border-gray-100 dark:border-gray-800 bg-blue-50/20 dark:bg-blue-950/10">
      <div className="flex items-center gap-4 px-8 py-4">
        ...
        <div className="flex items-center gap-1 flex-1 min-w-0">
          <OutlineBadge small label={`@${campaign.target_profiles[0]}`} />
        ...
        <div className="flex-[2] min-w-0 flex items-center gap-1.5">
          {campaign.comment && (
            <p className="text-xs text-gray-500 dark:text-gray-400 truncate min-w-0">{campaign.comment}</p>
          )}
      ...
      <CampaignProgressPanel campaign={campaign} onStatusChange={onStatusChange} />
    </div>
  );
}
...
function QueueRow({
  ...
  // When dragging, show an empty slot so the origin is obvious
  if (isDragging) {
    return (
      <div
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        {Object.keys(lockedBy).length > 0 ? (
          ...
          <button
            ...
            title={onStart ? "Start campaign" : "Auto-queue is running"}
          >
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
      ...
      <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Add a campaign to get started</p>
    </div>
  );
}
...
function EditSheet({
  ...
  open: boolean;
  onClose: () => void;
  onSave: (id: string, changes: Partial<CommentCampaign>) => void;
}) {
    ...
    <>
      ...
      >
        ...
        <div className="px-8 pt-8 pb-6 border-b border-gray-100 dark:border-gray-800 flex items-start justify-between flex-shrink-0">
          <div>
            <h3 className="text-sm font-medium text-gray-800 dark:text-gray-100">Edit Campaign</h3>
...
export default function CampaignQueueTab() {
  const [items, setItems] = useState<CommentCampaign[]>([]);
  ...
  // insertBeforeId: the id of the row to insert before ("END" = append at bottom)
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    setItems((data ?? []).map(mapDbToCampaign));
  }, []);
  ...
  const handleDragStart = (id: string, e: React.DragEvent) => {
    dragId.current = id;
    // Set preview position immediately so it appears at cursor from the first frame
    if (dragPreviewRef.current) {
      dragPreviewRef.current.style.transform =
        `translate(${e.clientX + 12}px, ${e.clientY - 22}px)`;
    }
    setIsDraggingId(id);
    // Suppress the browser's native ghost — all visual feedback stays inside the queue
  ...
  const handleDragOver = (e: React.DragEvent, rowId: string, nextRowId: string | null) => {
    ...
    // Determine insertion point by comparing cursor Y to row midpoint
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    const insertTarget = e.clientY < rect.top + rect.height / 2
      ? rowId                    // cursor in top half → insert before this row
      : (nextRowId ?? "END");    // cursor in bottom half → insert before next row (or at end)
  ...
  const handleStart = async (id: string) => {
    ...
    try {
      ...
      toast.success(`Campaign started on ${PLATFORM_LABELS[campaign.platform]}`);
      setItems((prev) =>
        prev.map((c) =>
          c.id === id ? { ...c, status: "in-progress" as CampaignStatus, updated_at: new Date().toISOString() } : c
        )
      );
    } catch {
      toast.error("Could not reach the bot server");
    }
  };
  ...
  const handleRemove = async (id: string) => {
    ...
    toast.warning("Campaign removed");
  };
  ...
  const handleRetry = async (id: string) => {
    ...
    setItems((prev) =>
      prev.map((c) =>
        c.id === id
          ? { ...c, status: "not-started" as CampaignStatus, updated_at: now }
          : c
      )
    );
    toast.success("Campaign moved back to queue");
  };

  const handleRerun = async (campaign: CommentCampaign) => {
    const newId = "campaign_" + crypto.randomUUID();
    const now = new Date().toISOString();
    const newPosition = notStarted.length + 1;
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        custom_comment: original.custom_comment,
        media_attachments: original.media_attachments,
        number_of_posts: original.number_of_posts,
    ...
    toast.success("Campaign duplicated and added to queue");
  };

  const handleEdit = (campaign: CommentCampaign) => {
    setEditingCampaign(campaign);
    setIsEditSheetOpen(true);
  };

  const handleSaveEdit = (id: string, changes: Partial<CommentCampaign>) => {
    setItems((prev) =>
      prev.map((c) =>
  ...
  const moveToEndOfQueue = useCallback(async (id: string) => {
    ...
    setItems((prev) =>
      prev.map((c) =>
        c.id === id
          ? { ...c, status: "not-started" as CampaignStatus, queue_position: newPos, updated_at: new Date().toISOString() }
          : c
      )
    );
  }, []);
  ...
  const advanceQueue = useCallback(async () => {
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((c) =>
          c.id === next.id
            ? { ...c, status: "in-progress" as CampaignStatus, updated_at: new Date().toISOString() }
            : c
        )
      );
    } catch {
      toast.error("Could not reach the bot server — stopping auto-queue");
      setAutoQueueActive(false);
      autoQueueActiveRef.current = false;
      autoQueueLockRef.current = null;
      consecutiveFailsRef.current = 0;
    ...
    <div className="flex flex-col -m-8">
      ...
      <div className="px-8 pt-8 pb-6 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-sm font-medium text-gray-800 dark:text-gray-100">Campaign Queue</h2>
      ...
      {activeTab === "queued" && (
        <>
          ...
          {filteredQueue.length === 0 && running.length === 0 ? (
            ...
            <>
              {/* Container: clears indicator on exit, accepts drop anywhere inside */}
              <div
                onDragLeave={(e) => {
      ...
      {/* Floating drag preview — mirrors the actual row so it's clear what's being moved */}
      <div
        ref={dragPreviewRef}
        ...
        {isDraggingId && (() => {
          ...
          return (
            <div className="flex items-center gap-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-5 py-3.5 shadow-xl shadow-gray-300/40 dark:shadow-black/40 w-[520px] opacity-95">
              ...
              {/* Targets */}
              <div className="flex items-center gap-1 flex-1 min-w-0">
                <OutlineBadge small label={`@${c.target_profiles[0]}`} />
                {c.target_profiles.length > 1 && (
                  <span className="text-[10px] text-gray-400 dark:text-gray-500">+{c.target_profiles.length - 1}</span>
                )}
              </div>
              {/* Comment */}
              <div className="flex-[2] min-w-0 flex items-center gap-1.5">
                {c.comment && <p className="text-xs text-gray-500 dark:text-gray-400 truncate min-w-0">{c.comment}</p>}
      ...
      {/* Invisible element used as drag ghost — suppresses browser's native ghost image */}
